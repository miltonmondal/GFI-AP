...
class Models:
    ...
    class _Agnostic_():
        ...
        def start_training(self, dataset, eval_freq, epoch, preprocessFn=None):
            if self._optim is None: raise ConnectionError('Optimizer not attached. Use attach_optimizer to connect optimizer.')
            if self._loss_fn is None: raise ConnectionError('Loss function not attached. Use attach_loss_fn to connect loss_fn')
            collate_loss, correct_predictions, total_processed = torch.zeros(1).cuda(), torch.zeros(1).cuda(), torch.zeros(1).cuda()
    ...
    class _ResNet_(nn.Module, _Agnostic_):
        ...
        def dry_run(self, dataset, num_iterations=10):
            if self._optim is None: raise ConnectionError('Optimizer not connected. Use attach_optimizer to connect optimizer.')
            iteration = 0
            for data, labels in dataset.train_images:
                data, labels = data.cuda(), labels.cuda()
                output = self.forward(data)
...
class Datasets:
    ...
    def __init__(self, dataset, batch_size):
        ...
        self.train_images = torch.utils.data.DataLoader(self._train_dataset, batch_size=batch_size, shuffle=True, num_workers=8)
        self.eval_images = torch.utils.data.DataLoader(self._eval_dataset, batch_size=batch_size, shuffle=False, num_workers=8)
