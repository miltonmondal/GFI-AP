...
def thl_prune(p, trial, epochs, lear_change_freq, lear1, lr_divide_factor, lr_change_freq):
    ...
    for layer in range(net.max_layers()- V.ig_l):
        j = layer
        for k in range(layer, layer + 3):
            no_filters = np.int(p * net.max_filters(layer=k))
            print("No. of filters for pruning:", no_filters)
            for i in range(no_filters):
                net.prune(layer=k, filter=i, verbose=False)
            filter_left = net.max_filters(layer=k)
            print("filter left", filter_left)

        os.makedirs(V.base_path_results+"/Pruned_States/start_layer_" + str(j), exist_ok=True)
        print('Saving Pruned state')
        net.save_pruned_state(V.base_path_results+"/Pruned_States/start_layer_" + str(j) + "/importance_retained_state-001")
        ...
        os.makedirs(V.base_path_results+"/Fine_Tune_Checkpoints/start_layer_" + str(j), exist_ok=True)
        net = api.Models(model=V.model_str, num_layers=V.n_l, num_transition_shape=1 * 1, num_linear_units=512,
                         num_class=V.n_c).net()
        net.restore_pruned_state(V.base_path_results+"/Pruned_States/start_layer_" + str(j) + "/importance_retained_state-001")
        ...
        for e in range(0, epochs, lr_change_freq):
            ...
            net.save_checkpoint(V.base_path_results+"/Fine_Tune_Checkpoints/start_layer_" + str(j) + "/epoch_" + str(e) + '.ckpt')
